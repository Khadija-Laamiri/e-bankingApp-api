pipeline {
    ...
    environment {
        ...
        APP_REGISTRY = "enamifatimazahrae2001/facture"
        MANIFEST_FILE = "facture-service/facture-deployment.yml"
    }
                        ...
                        withCredentials([string(credentialsId: 'github_manifest', variable: 'GITHUB_TOKEN')]) {
                            ...
                            dir('e-banking-api-manifest') {
                                bat """
                                   ...
                                   powershell "(Get-Content -Path '${MANIFEST_FILE}') -replace 'enamifatimazahrae2001/facture.*', 'enamifatimazahrae2001/facture:${IMAGE_TAG}' | Set-Content -Path '${MANIFEST_FILE}'"
                                    git diff
                                    git add .
                                    git commit -m "Update facture service image version to ${IMAGE_TAG}"
